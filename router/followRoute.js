...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        const result = await User.findOneAndUpdate({ username: follow }, { $push: { followers: req.session.user._id } }, { new: true });
        if (!result) {
            throw { error: "Something went wrong", statusCode: 500 };
        } else {
            const result2 = await User.findByIdAndUpdate(req.session.user._id, { $push: { following: result._id } }, { new: true });
            if (!result2) {
                throw { error: "Something went wrong", statusCode: 500 };
            } else {
